...
**Goal:** quiet by default. Simple per-topic control and easy privacy. There is no standalone Alerts screen anymore.
**Style:** LinkedIn-style settings list. See [style-guide.md](style-guide.md).

## Where notifications live now
The old full-page “Alerts” list has been removed. Tapping the bell in the Home header opens a small bottom sheet with the same recent items (connection request, event RSVP, event reminder, market message, club update), but each row is a **direct link to the page it’s about** — the event detail, the message thread, the club page — instead of staying inside a generic feed. See [Home](02-home-mobile.md#screen-1--home-feed) for the sheet itself. This page is reached from the gear icon on your own profile, or from the gear next to your picture in the desktop top bar (mobile doesn’t have a persistent top bar, so it’s reached via Profile there).
...
- **Reminders** (card of switch rows; each row = title + description + switch):
...
- Defaults are low-noise: club announcements are rate-limited to 1 per week.
